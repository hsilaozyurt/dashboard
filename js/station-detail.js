/* ================================================================
   ...
   - Adds safe back button
   - Preserves router page classes
   ================================================================ */
...
SRD.STATION_DETAIL = (function () {
  ...
  function render(container, data, loc) {
    ...
    var station = data.stations.find(function (s) {
      return s.loc === loc;
    });

    if (!station) {
      container.innerHTML =
        '<div style="padding:40px;color:#4e6a84">Station not found: ' +
        escapeHTML(loc) +
        '</div>';
      return;
    }
    ...
    /*
      Önemli:
      container.className = 'detail-page' kullanmıyoruz.
      Çünkü bu, router için gerekli olan "page" ve "srd-active"
      classlarını silebilir.
    */
    container.classList.add('detail-page');
    ...
    left.innerHTML = [
      '<div class="detail-iata-badge">' + escapeHTML(loc) + '</div>',
      '<div>',
        '<div class="detail-station-name">' + escapeHTML(loc) + ' Station</div>',
        '<div class="detail-station-sub">Risk summary · ' + escapeHTML(station.count) + ' recorded events</div>',
      '</div>'
    ].join('');
    ...
    backBtn.innerHTML = [
      ...
      '</svg>',
      escapeHTML(t('backToStations'))
    ].join(' ');

    backBtn.onclick = function () {
      SRD.ROUTER.go('stations');
    };
    ...
    var kpis = [
      {
        cls: 'blue',
        label: 'Total Events',
        val: D.fmtInt(station.count),
        sub: 'Recorded occurrences'
      },
      {
        cls: 'orange',
        label: 'Composite Score',
        val: Number(station.composite || 0).toFixed(2),
        sub: '(Σ Risk / Flights) × 100'
      },
      {
        cls: 'green',
        label: 'Total Flights',
        val: D.fmtInt(station.flights),
        sub: 'Flight count'
      },
      {
        cls: 'purple',
        label: 'Risk Level',
        val: station.compLevel,
        sub: 'Composite category'
      }
    ];
    ...
    trendCard.innerHTML = [
      '<div class="card-header">',
        '<div>',
          ...
          '<div class="card-sub">' + escapeHTML(loc) + ' — monthly event count</div>',
        '</div>',
      '</div>',
    ...
    var mdata = labels.map(function (k) {
      return monthMap[k];
    });
    ...
    var ddata = CATS.map(function (c) {
      return station.catCounts[c] || 0;
    });

    var colors = CATS.map(function (c) {
      return D.SEV_COLOR[c];
    });

    var total = ddata.reduce(function (a, b) {
      return a + b;
    }, 0);
      ...
      LABELS.forEach(function (lbl, i) {
        ...
        row.innerHTML = [
          ...
          '<div class="legend-right">',
            '<span class="legend-val">' + escapeHTML(ddata[i]) + '</span>',
            '<span class="legend-pct">%' + escapeHTML(pct) + '</span>',
          '</div>'
        ].join('');
